...
const jqData = getAccurateJieQi(new Date(y, m - 1, d, h, min));
        ...
        if (qimenMethod === 'yinpan') {
            ...
            const yuan = ["子", "午", "卯", "酉"].includes(fuTouBranch) ? "上元" : (["寅", "申", "巳", "亥"].includes(fuTouBranch) ? "中元" : "下元");
        ...
        if (pNum === 5) {
            resGod = ""; resStar = []; resDoor = ""; resHStems = [earthPan[5]]; resEStems = [earthPan[5]];
        }

        const isMenPoFn = (door, num) => {
            const fire = num===9, water = num===1, earth = num===2||num===8, metal = num===6||num===7, wood = num===3||num===4;
        ...
        let isMenPo = false; let hasJiXing = false; let hasRuMu = false;

        if (pNum !== 5) {
            isMenPo = isMenPoFn(resDoor, evalPNum);
        ...
        if (((dStemTX === '甲' || dStemTX === '己') && (timeGZ_string === '己巳')) || ((dStemTX === '乙' || dStemTX === '庚') && timeGZ_string === '甲申') || ((dStemTX === '丙' || dStemTX === '辛') && timeGZ_string === '甲午') || ((dStemTX === '丁' || dStemTX === '壬') && timeGZ_string === '甲辰') || ((dStemTX === '戊' || dStemTX === '癸') && timeGZ_string === '甲寅')) {
            isTianXian = true;
        }

        return {
            god: resGod, star: resStar, door: resDoor, hStems: resHStems, eStems: resEStems,
            bazi: `${yearGZ}年 ${monthGZ}月 ${dayGZ}日 ${timeGZ_string}時`,
            yStem: yearGZ.substring(0, 1), mStem: monthGZ.substring(0, 1), dStem: dayStem, hStem: timeStem,
            isKong: isKong, isMa: isMa, isMenPo: isMenPo, hasJiXing: hasJiXing, hasRuMu: hasRuMu, isChong: isChong,
            ziMark: ziMark, isTianXian: isTianXian, isWuBuYu: isWuBuYu
        };
...
function executeRealSearch() {
    ...
    const criteria1 = {
        gods: getChecked('chk-group-god-1'), stars: getChecked('chk-group-star-1'), doors: getChecked('chk-group-door-1'),
        hStems: getChecked('chk-group-hstem-1'), eStems: getChecked('chk-group-estem-1'),
        kong: document.getElementById('chk-kong-1')?.checked, noKong: document.getElementById('chk-nokong-1')?.checked,
        ma: document.getElementById('chk-ma-1')?.checked, noMa: document.getElementById('chk-noma-1')?.checked,
        jiXing: document.getElementById('chk-jixing-1')?.checked, noJiXing: document.getElementById('chk-nojixing-1')?.checked,
        ruMu: document.getElementById('chk-rumu-1')?.checked, noRuMu: document.getElementById('chk-norumu-1')?.checked,
        chong: document.getElementById('chk-chong-1')?.checked, noChong: document.getElementById('chk-nochong-1')?.checked,
        menPo: document.getElementById('chk-menpo-1')?.checked, noMenPo: document.getElementById('chk-nomenpo-1')?.checked,
        tianXian: document.getElementById('chk-tianxian-1')?.checked, wuBuYu: document.getElementById('chk-wubuyu-1')?.checked
    };

    const criteria2 = pNum2 !== "0" ? {
        gods: getChecked('chk-group-god-2'), stars: getChecked('chk-group-star-2'), doors: getChecked('chk-group-door-2'),
        hStems: getChecked('chk-group-hstem-2'), eStems: getChecked('chk-group-estem-2'),
        kong: document.getElementById('chk-kong-2')?.checked, noKong: document.getElementById('chk-nokong-2')?.checked,
        ma: document.getElementById('chk-ma-2')?.checked, noMa: document.getElementById('chk-noma-2')?.checked,
        jiXing: document.getElementById('chk-jixing-2')?.checked, noJiXing: document.getElementById('chk-nojixing-2')?.checked,
        ruMu: document.getElementById('chk-rumu-2')?.checked, noRuMu: document.getElementById('chk-norumu-2')?.checked,
        chong: document.getElementById('chk-chong-2')?.checked, noChong: document.getElementById('chk-nochong-2')?.checked,
        menPo: document.getElementById('chk-menpo-2')?.checked, noMenPo: document.getElementById('chk-nomenpo-2')?.checked,
        tianXian: document.getElementById('chk-tianxian-2')?.checked, wuBuYu: document.getElementById('chk-wubuyu-2')?.checked
    } : null;
    ...
    const hourSlots = [
        { h: 0, min: 30, branch: '子', name: '早子' }, { h: 2, min: 30, branch: '丑', name: '丑' },
        { h: 4, min: 30, branch: '寅', name: '寅' }, { h: 6, min: 30, branch: '卯', name: '卯' },
        { h: 8, min: 30, branch: '辰', name: '辰' }, { h: 10, min: 30, branch: '巳', name: '巳' },
        { h: 12, min: 30, branch: '午', name: '午' }, { h: 14, min: 30, branch: '未', name: '未' },
        { h: 16, min: 30, branch: '申', name: '申' }, { h: 18, min: 30, branch: '酉', name: '酉' },
        { h: 20, min: 30, branch: '戌', name: '戌' }, { h: 22, min: 30, branch: '亥', name: '亥' },
    ...
    setTimeout(() => {
        ...
        for(let i = 0; i < 180; i++) {
            let y = currentD.getFullYear(); let m = currentD.getMonth() + 1; let d = currentD.getDate();
            
            for(let slot of hourSlots) {
                let h = slot.h; let min = slot.min; 
                ...
                if (matchOverall) {
                    matchedResults.push({
                        ...
                        bazi: elements1.bazi, ziMark: elements1.ziMark
                    });
                }
            }
        ...
        if (matchedResults.length === 0) {
            ...
            matchedResults.forEach(item => {
                ...
                btn.className = "filter-result-btn w-full text-left p-3 border border-amber-200/60 rounded-xl hover:bg-amber-50 focus:bg-amber-100 active:scale-[0.99] transition duration-150 shadow-sm text-sm bg-white block mb-2";
                btn.innerHTML = `
                ...
                `;
                btn.onclick = () => {
                    document.querySelectorAll('.filter-result-btn').forEach(el => {
                        el.classList.remove('bg-amber-100', 'border-amber-400', 'shadow-md', 'ring-2', 'ring-amber-300');
                        el.classList.add('bg-white', 'border-amber-200/60');
                    });
                    btn.classList.remove('bg-white', 'border-amber-200/60');
                    btn.classList.add('bg-amber-100', 'border-amber-400', 'shadow-md', 'ring-2', 'ring-amber-300');
                    ...
                    if (window.innerWidth < 1024) {
                        setTimeout(() => {
                            ...
                            const jumpBtn = document.getElementById('btn-jump-filter');
                            if (jumpBtn) jumpBtn.classList.remove('translate-y-20', 'opacity-0', 'pointer-events-none');
                        }, 150);
                    }
                };
...
function searchBaziDates(tgtY, tgtM, tgtD, tgtH) {
    ...
    for (let i = 0; i < 60; i++) {
        ...
        if (noonS.getLunar().getDayInGanZhiExact() === tgtD) { firstMatch = d; break; }
    }
    ...
    if (firstMatch) {
        ...
        while (curr <= end) {
            let y = curr.getFullYear(); let m = curr.getMonth() + 1; let d = curr.getDate();
            for (let slot of slotsToCheck) {
                let testHour = slot.h; let testMin = slot.min;
                ...
                if (yGZ === tgtY && mGZ === tgtM) {
                    ...
                    if (testTimeGZ === tgtH) results.push(new Date(y, m - 1, d, testHour, testMin));
                }
            }
...
function applyBaziResult(y, m, d, h, min) {
    ...
    if (isBaziMode) toggleInputMode(); // toggleInputMode 在 app.js 中
    ...
    updateJieQiYearBtn(); // updateJieQiYearBtn 在 app.js 中
    generatePan();
}