...
function getTrueSolarTime(baseDate, cityKey) {
    if (cityKey === "none" || !CITY_COORDINATES[cityKey]) {
        return {
    ...
    const stdLon = city.tz * 15; // 該時區的標準經度 (例如 UTC+8 為 120度)
    const lonDiff = city.lon - stdLon; 
    const lonOffsetMinutes = lonDiff * 4; // 每差1度經度，時間差4分鐘

    // 計算均時差 (Equation of Time)，使用天文近似公式
...
function toggleInputMode() {
    ...
    if (isBaziMode) {
        ...
        if (locSels) locSels.classList.add('hidden'); // 八字模式不需真太陽時
    } else {
        btn.innerText = "四柱方式";
...
function toggleFilterPanel() {
    ...
    if (isFilterPanelOpen) {
        ...
        if (window.innerWidth < 1024) {
            setTimeout(() => filterPanel.scrollIntoView({ behavior: 'smooth', block: 'start' }), 150);
        }
    } else {
...
const locSel = document.getElementById('sel-location');
    ...
    if (urlParams.has('note')) {
        const rawNote = urlParams.get('note');
        let decompressed = "";
        try {
            if (typeof LZString !== 'undefined') decompressed = LZString.decompressFromEncodedURIComponent(rawNote) || decodeURIComponent(rawNote);
            else decompressed = decodeURIComponent(rawNote);
        } catch (err) { decompressed = rawNote; }
        const noteInput = document.getElementById('note-input');
        if (noteInput) {
            noteInput.value = decompressed;
            updateNoteCounter();
        }
        const accNote = document.getElementById('accordion-notes');
        if (accNote) accNote.open = true;
    }
}
